...
**가격 기준**: Mode2 watcher에서 `buy_target_price` / `resistance_1_price` / `resistance_2_price` 조회. Mode2 미등록 시 trade_watchlist의 `buy_price` / `exit_price` fallback.
...
- **단일가 매매일 자동 제외**: 봉 간격이 25분+ 비율 > 50%이면 `DANILGA` 태그로 표시 후 스킵
...
- 단일가 매매일(봉 간격 25분+ > 50%) → 스킵
- C2 지지가 산출 → 시그널 감지 → 텔레그램 즉시 알림 + DB 저장
...
**단기과열 억제 폐기**: exit_date 다음날부터 즉시 시그널 허용. 단일가 매매일(30분 단위 단일가 거래)만 제외. 한컴위드 4/21~4/23이 단일가 구간이었음 — 이 경우 시간 연속성이 없어 3분봉 시그널 의미 없음.

**단일가 매매일 감지**: 3분봉 조회 후 봉 간격 25분+ 비율이 50% 초과이면 단일가로 판단 → 스킵. `DANILGA` 타입으로 백테스트 결과에 표시.

**overheat_date**: `exit_date`를 직접 사용 (자동탐지 폐기 — 전체 일봉에서 탐색하면 수년 전 폭등일 오탐 발생).

**C2 dedup**: 같은 `support_price` ±100원은 하루에 첫 감지만. 반복 알림 방지.

**C2 지지가 계산 기준**: `exit_date` 이후 일봉에서 쌍바닥 탐색. H+ 등급 기준은 `resistance_1_price` ±2% 이내 (이전: `exit_price` — Mode2 가격 체계로 전환됨).

**Type B 조건**: +2% 이상 돌파 (이전 +5%). B-r1(1차저항), B-r2(2차저항)으로 분리. 같은 가격대(±2%) 반복 알림 방지.

**가격 체계 전환 (2026-05)**: 기존 `buy_price`/`exit_price` → Mode2 `buy_target_price`/`resistance_1_price`/`resistance_2_price`. trade_watchlist에 Mode2가 없으면 `buy_price`/`exit_price` fallback.
